...
public class EvalPlayer : Player
{
    LegalMoveGenerator lmg;

    public EvalPlayer(LoRBoard board, int playerNumber, Deck deck)
    {
        this.board = board;
    ...
    {
        return unit.power + unit.health + (unit.HasKeyword(Keyword.Tough) ? 1 : 0) + (unit.HasKeyword(Keyword.Challenger) ? 1 : 0) + 3;
    }
    ...
    public Dictionary<string, float> EvaluateLoRBoardState(LoRBoard board)
    {
        ...
        for (int i = 0; i <  board.spellStack.spells.Count; i++) //spell on stack value
        {
            if (board.spellStack.castingOrder[i] == playerNumber)
            {
                ...
                {
                    case "Reinforcements":
                        UpdateEvalDictValue("Reinforcements", (8 + 3) * 2);
                        break;
                    case "For Demacia!":
                        if (evalSide.hasAttackToken) { UpdateEvalDictValue("For Demacia!", evalSide.bench.units.Count * 5); }
                        break;
                    case "Succession":
                        UpdateEvalDictValue("Succession", 6 + 3);
                        break;
        ...
        if ((board.passCount == 1 && board.activePlayer != playerNumber && !board.inCombat) || (evalSide.mana.manaGems == board.roundNumber &&  opposingSide.mana.manaGems == board.roundNumber)) // for pass burn
        {
            ...
            UpdateEvalDictValue("Mana (Passing)", Math.Min(3 * spellManaValue, (evalSide.mana.manaGems + evalSide.mana.spellMana) * spellManaValue));
        }
        else
        {
            UpdateEvalDictValue("Benched Units", evalSide.bench.units.Sum(x => UnitValue(x)));
            UpdateEvalDictValue("Mana", evalSide.mana.manaGems + evalSide.mana.spellMana * spellManaValue);
        }
